...
impl DbFiles {
    ...
    fn current_db_file(&self) -> Option<PathBuf> {
        match self.state() {
...
impl Component<Msg, NoUserEvent> for DbFiles {
        ...
        match ev {
            Event::Keyboard(KeyEvent {
                ...
                self.current_db_file()
                    .map(|x| Msg::LoadDb(LoadDbMsg::DbChanged(x)))
            }
            ...
            Event::Keyboard(KeyEvent { code: Key::Esc, .. }) => {
                Some(Msg::LoadDb(LoadDbMsg::GoToMenu))
            }
            Event::Keyboard(KeyEvent {
                code: Key::Enter, ..
            }) => self
                .current_db_file()
                .map(|x| Msg::LoadDb(LoadDbMsg::LoadDb(x))),
            _ => None,
        }
    }
}
