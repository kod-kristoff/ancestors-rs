...
// use crate::app::session::{Action, ExploreAction, FightAction, Session};
    ...
    fn on(&mut self, ev: Event<NoUserEvent>) -> Option<Msg> {
        match ev {
            ...
            Event::Keyboard(KeyEvent {
                ...
                .selected_action()
                .map(|x| Msg::Session(SessionMsg::ActionSelected(x))),
            Event::Keyboard(KeyEvent {
                code: Key::Char('e'),
                modifiers: KeyModifiers::NONE,
            }) => Some(Msg::Session(SessionMsg::ShowInventory)),
            Event::Keyboard(KeyEvent {
                code: Key::Char('s'),
                modifiers: KeyModifiers::NONE,
            }) => Some(Msg::Session(SessionMsg::ShowSaveFileName)),
            Event::Keyboard(KeyEvent { code: Key::Esc, .. }) => {
                Some(Msg::Session(SessionMsg::ShowQuitPopup))
            }
            _ => None,
        }
    }
